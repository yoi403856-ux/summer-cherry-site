...
const PARENT = `{ _id, name, call, "slug": slug.current, role, color, images }`
const KITTEN = `_id, name, "slug": slug.current, litter, litterEn, color, sex, born, status, description, descriptionEn, images, order, father->${PARENT}, mother->${PARENT}`
...
export async function getKittensByStud(studId) {
  ...
  return safeFetch(
    `*[_type == "kitten" && (father._ref == $id || mother._ref == $id)] | order(order asc, born desc){
      _id, name, "slug": slug.current, color, status, images
    }`,
    { id: studId },
    []
  )
